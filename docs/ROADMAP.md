...
- [ ] **이메일 가입 플로우에 생년월일(또는 나이) 입력 필수화 + 만 14세 미만 가입 차단** —
      현재 `components/sign-up-form.tsx`(이메일 가입)에는 나이 입력 자체가 없고, 나이
      검증(`lib/schemas/profile.ts`의 `age >= 14` 룰)은 **OAuth 가입 후 프로필 설정 단계에만**
      연결되어 있어 이메일 가입 사용자는 나이 검증을 아예 거치지 않는다. 두 가입 경로 모두
      동일하게 막아야 한다.
- [ ] 만 14세 미만으로 판별되면 법정대리인 동의 절차를 별도로 구현하기보다 **가입 자체를
      차단**하는 정책 채택(대부분의 국내 서비스가 택하는 실무 방식 — 동의 절차 구현·검증
      비용이 훨씬 크다)
...
- [ ] **이용약관** 페이지 신설 + 회원가입 폼에 **필수 동의 체크박스** 추가 (현재 `sign-up-form.tsx`에
      동의 체크박스 자체가 없음)
- [ ] **개인정보처리방침** 페이지 신설 — 수집 항목(닉네임/이메일/성별/나이/대화·이미지 등),
      수집 목적, 보유·이용 기간, 파기 절차, **제3자 제공/처리위탁 현황**(Supabase: 인증·DB·
      Storage, Vercel: 호스팅 등)을 명시. 현재 `docs/`, `app/` 어디에도 관련 문서·페이지 없음
- [ ] **청소년보호정책** 페이지 신설 — 정보통신망법상 청소년유해매체물 관련 표시·조치 의무
- [ ] 위탁업체가 해외 법인(Supabase Inc., Vercel Inc.)이므로 **개인정보 국외이전 고지** 필요
      여부를 자문받아 명시 (DB/Storage 리전은 서울(ap-northeast-2)이지만 처리위탁 계약 주체는
      해외 법인)
- [ ] 대화 내용이 **수사기관 요청 시 제공될 수 있음**을 약관에 명시 (통신비밀보호법 관련 근거
      마련 — 현재 아무 문서에도 이 내용 없음)

### ③ 콘텐츠·안전 (아동·청소년의 성보호에 관한 법률, 정보통신망법 — 가장 중요)

- [ ] **닉네임·방 제목 금칙어 자동 필터링** — 현재 `lib/schemas/room.ts`의 방 제목 검증은
      길이 제한만 있고, 코드베이스 전체에 "금칙어/profanity/banned_word" 필터링 로직이
      전혀 없음(조사 확인). 성적 표현·연락처(전화번호/메신저 ID) 유도성 문구·불법 거래
      암시 문구를 최소한 자동 차단
- [ ] **메시지 실시간 금칙어 필터링 또는 자동 플래깅** — 현재는 사용자 신고(`app/actions/reports.ts`,
      `message`/`user`/`room`/`random_session` 4종 지원)에만 의존. 신고가 들어오기 전에는
      관리자가 능동적으로 걸러낼 방법이 없음
...
- [ ] **아동성착취물(CSAM) 발견 시 즉시 삭제·전송 차단 + 수사기관 신고** 프로세스를 관리자
      운영 매뉴얼로 문서화 — 아동·청소년의 성보호에 관한 법률상 온라인서비스제공자의 신고
      의무 대상이며, 발견 후 처리 지연 자체가 법적 리스크. `/admin` 신고 큐(Phase 7.5 구현
      완료)를 이 목적에도 쓸 수 있도록 처리 우선순위·에스컬레이션 절차를 정의
- [ ] 미성년자 대상 성범죄 정황(예: 미성년자임을 밝힌 대화 중 성적 대화 유도)을 신고 접수 시
      관리자가 우선 처리하도록 `/admin/reports` 처리 정책에 명시
...
- [ ] **Phase 8.5(법적/규정 준수) 필수 항목 완료 확인 — 실제 사용자에게 공개하기 전 반드시 선행**
