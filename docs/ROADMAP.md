...
- [ ] `profiles.role` 기반 관리자 판별 (기존 스타터킷의 `role` 컬럼 재사용, `admin` 값 정의)
- [ ] `/admin/*` 라우트 미들웨어 보호 (비관리자 접근 시 404 또는 홈 리다이렉트 — 관리자 경로 존재 자체를 노출하지 않는 쪽 검토)
- [ ] 모든 관리자 조회/액션에 서버 측 관리자 권한 재검증 (클라이언트 판별만 믿지 않음)
- [ ] 관리자 전용 조회를 위한 RLS 정책 또는 SECURITY DEFINER 함수 설계 (일반 사용자 RLS를 느슨하게 만들지 않는 방향)

### 대화 내용 조회

- [ ] 방채팅 조회 — 방 목록/검색, 방별 메시지 타임라인, 참여자 목록
- [ ] 랜덤채팅 조회 — 활성 세션 및 `random_session_archives` 아카이브 검색(참여자 id/기간 기준)
- [ ] 방채팅 아카이브(`room_archives`) 조회 — 삭제된 방의 스냅샷
- [ ] 전송된 이미지 확인 — 메시지에 첨부된 이미지 썸네일/원본 열람 (Storage 관리자 접근 경로 설계)
...
- [ ] 가입 회원 목록 (`profiles`) — 닉네임/이메일/가입일/최근 접속(`last_seen_at`) 조회, 검색·정렬
- [ ] 회원 상세 — 참여 중인 방, 최근 대화 이력, 신고 이력
- [ ] 게스트(`guest_profiles`) 현황 조회 — 활성 게스트 수, 자동 정리 배치 동작 확인용
- [ ] 이용 제재 — 계정 정지/차단, 강제 탈퇴 (사유·기간 기록)
...
- [ ] 대시보드 — 가입자 수, DAU, 진행 중인 방/랜덤 세션 수, 일별 메시지 수 등 기본 지표
- [ ] 방 강제 삭제 / 메시지 강제 삭제 (부적절 콘텐츠 대응) — 아카이브 우회 문제는 위 "사용자 관리" 섹션의 경고 참고
- [ ] 신고 기능 및 신고 처리 큐 (사용자 신고 → 관리자 검토 → 조치) — 신고 접수 UI는 일반 사용자 화면에도 필요
- [ ] 관리자 조치 감사 로그 (누가·언제·무엇을 — 조회 자체도 기록 대상으로 할지 결정)
- [ ] 시스템 상태 점검 — pg_cron 배치(게스트 정리, 아카이브 정리, 유령 세션 정리) 최근 실행 결과 확인

**연관 PRD**: §7.4 후속 검토(운영자 관리 화면), §7.1 권한 검증

**완료 조건**: 관리자 계정으로만 `/admin`에 접근 가능하고(일반 회원·게스트는 완전 차단, 서버 측에서도 재검증됨), 방채팅·랜덤채팅의 대화 내용과 전송 이미지를 조회할 수 있으며, 회원 목록 조회와 최소 1개 이상의 제재 조치(정지 또는 강제 탈퇴)가 동작한다.
