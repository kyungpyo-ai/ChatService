...
- [ ] **탈퇴 시 아카이브 우회 문제 해결** — 아래 참고
- [ ] 메시지 전송/방 생성/**이미지 업로드** 요청 횟수 제한(rate limit) — 이미지 업로드는 게스트도 가능해 스팸·불법 이미지의 주요 경로이므로 Phase 6에서 이월된 항목
...
> **⚠️ 착수 전 Phase 7의 선행 처리 확인**: 관리자의 "강제 탈퇴"·"방 강제 삭제"는 §Phase 7에서
> 언급한 `rooms.owner_id ON DELETE CASCADE` 문제(계정 삭제/방 삭제가 `leave_room()`의 아카이브
> 로직을 거치지 않아 대화 기록이 통째로 증발하는 문제)와 정확히 같은 경로를 탄다. Phase 7에서
> 트리거 방식으로 해결했다면 이 항목은 자동으로 안전하지만, **아직 해결 전이라면 관리자 기능을
> 먼저 만들지 말고 §Phase 7 선행 처리부터 끝낼 것** — 신고 대응용 관리자 화면이 정작 그 신고의
> 증거를 지우는 도구가 되는 모순을 피하기 위함이다.
