...
## Phase 7.7 — 법적/규정 준수 (배포 전 필수)

> ⚠️ **면책 고지**: 이 섹션은 법률 자문이 아니라, 익명·랜덤 채팅 + 이미지 전송 서비스가
> 일반적으로 부딪히는 국내(대한민국) 규제 이슈를 실제 배포 전 점검용으로 정리한 체크리스트다.
> 이 서비스는 **로그인 없이 익명으로 실시간 대화하고 이미지를 주고받을 수 있다는 특성상**,
> 국내에서 실제로 사회적 물의를 일으킨 유사 서비스 사례(음란 채팅, 미성년자 대상 성범죄
> 알선 등)와 같은 리스크 범주에 속한다. 정식 서비스 오픈 전 **반드시 변호사(가능하면
> 정보통신법/개인정보보호법 실무 경험이 있는 자문)의 검토를 받을 것** — 특히 아래 "①
> 콘텐츠·안전" 항목은 형사처벌 대상이 될 수 있는 영역이라 개발팀 판단만으로 결정하면 안 된다.

### ① 가입 연령 제한 (개인정보보호법 — 아동의 개인정보 보호)

- [x] **이메일 가입 플로우에도 나이 검증 필수화 + 만 14세 미만 가입 차단** (완료, 2026-08-15) —
      "닉네임 없으면 setup-profile로 보낸다" 가드를 OAuth 콜백 전용에서 `app/(main)/layout.tsx`
      공용 가드로 옮겨, 이메일 가입 사용자도 로그인 즉시 나이 검증(`ageSchema`, 만 14~120세)을
      거치지 않으면 서비스를 이용할 수 없게 됨(§DEVELOPMENT_PLAN 7.7.1)
- [x] 만 14세 미만은 가입 자체를 차단하는 정책 채택 (완료, 2026-08-15) — `ageSchema` refine이
      이미 이 정책대로 동작, 위 가드 통합으로 이메일 가입 경로에도 동일 적용됨
- [ ] (권장, MVP 이후) 자기 입력 나이/생년월일만으로는 위조가 쉬우므로, 성장 단계에서
      휴대폰 본인인증(또는 아이핀) 연동 검토 — 익명 랜덤채팅 서비스일수록 자기신고식 나이
      입력의 실효성이 낮다는 점을 감안

### ② 약관·정책 페이지 (정보통신망법·개인정보보호법 — 필수 게시 의무)

- [x] **이용약관** 페이지 신설(`app/legal/terms/page.tsx`) + **필수 동의 체크박스** 추가
      (완료, 2026-08-15) — 회원가입 폼 자체가 아니라, 가입 경로와 무관하게 반드시 거치는
      setup-profile 공용 관문에 동의 체크박스를 붙임(§DEVELOPMENT_PLAN 7.7.1). 단, 문구는
      법률 검토 전 초안이며 확정 아님
- [x] **개인정보처리방침** 페이지 신설(`app/legal/privacy/page.tsx`) (완료, 2026-08-15) —
      수집 항목·목적·보유기간·제3자 위탁 현황(Supabase/Vercel) 골격 작성. 문구는 법률 검토
      전 초안
- [x] **청소년보호정책** 페이지 신설(`app/legal/youth-protection/page.tsx`) (완료, 2026-08-15) —
      책임자 연락처는 "추후 공지"로 placeholder, 실제 지정은 운영 결정 사항으로 남김
- [ ] 위탁업체가 해외 법인(Supabase Inc., Vercel Inc.)이므로 **개인정보 국외이전 고지** 필요
      여부를 자문받아 명시 (DB/Storage 리전은 서울(ap-northeast-2)이지만 처리위탁 계약 주체는
      해외 법인)
- [x] 대화 내용이 **수사기관 요청 시 제공될 수 있음**을 약관 초안에 명시 (완료, 2026-08-15) —
      `app/legal/terms/page.tsx` 제5조. 최종 문구는 법률 검토 후 확정

### ③ 콘텐츠·안전 (아동·청소년의 성보호에 관한 법률, 정보통신망법 — 가장 중요)

- [x] **닉네임·방 제목 금칙어 자동 필터링** (완료, 2026-08-15) — `lib/utils/content-filter.ts`
      신규(`containsBannedWord`), `lib/schemas/room.ts`(방 제목)·`lib/schemas/profile.ts`
      (닉네임)에 refine으로 연결. 초기 리스트는 성적 표현·연락처 유도 패턴 위주 최소 구성
- [ ] **메시지 실시간 금칙어 필터링 또는 자동 플래깅** — 닉네임·방 제목과 달리 메시지
      전송(`app/actions/messages.ts`)에는 필터를 적용하지 않기로 결정(2026-08-15, 사용자
      요청) — 실시간 대화 흐름을 막지 않기 위함. 현재는 사용자 신고(`app/actions/reports.ts`)
      에만 의존
- [ ] **이미지 업로드 자동 검열 도입 검토** — 현재 `app/actions/chat-images.ts`는 확장자/용량/
      참여자 검증만 하고 콘텐츠 자체를 검사하지 않는다. 익명 사용자(게스트)도 이미지를
      올릴 수 있는 구조라 리스크가 특히 크다 — 최소한 성인 콘텐츠 자동 분류(예: AWS
      Rekognition Moderation, Google Cloud Vision SafeSearch 등 상용 API) 연동을 검토할 것
- [x] **아동성착취물(CSAM) 발견 시 즉시 삭제·전송 차단 + 수사기관 신고** 절차 목차 작성
      (완료, 2026-08-15) — `docs/CONTENT_SAFETY_PROCEDURE.md` 신규. 즉시 실행 가능한 조치
      (`/admin` 강제 삭제·세션 종료 함수 연결)까지는 채웠고, 원본 격리 경로·관계기관 신고
      채널·연락처는 법률 검토 후 확정하는 항목으로 명시적으로 남겨둠(미확정)
- [x] 미성년자 대상 성범죄 정황(예: 미성년자임을 밝힌 대화 중 성적 대화 유도)을 신고 접수 시
      관리자가 우선 처리하도록 `/admin/reports` 처리 정책에 명시 (완료, 2026-08-15) —
      `docs/CONTENT_SAFETY_PROCEDURE.md` 2절. 새 테이블/컬럼 없이 운영 정책 문서로만 처리

### ④ 운영 체계

- [ ] **청소년보호책임자 지정** 검토 및 서비스 내 공개(정보통신망법 — 일정 매출·이용자 규모
      이상 사업자 대상 의무. MVP 초기엔 기준 미달일 수 있으나 정책·연락처 페이지는 미리
      준비해두는 편이 안전)
- [ ] **부가통신사업 신고**(전기통신사업법) 필요 여부 검토 — 매출액/일평균 이용자 수 기준
      초과 시 신고 의무 발생. 서비스 성장 단계에서 재점검
- [ ] 최소한의 **접속 로그(IP 등) 보관 정책** 수립 — 현재 `lib/supabase/middleware.ts`에는
      디버그용 `console.log` 외 별도 접속 로그 저장이 없음. 수사기관 협조·악성 이용자 추적을
      위해 최소 기간 보관 정책을 검토(개인정보보호법상 목적 외 과다 수집이 되지 않도록 보관
      기간·목적을 명확히 한정)
- [ ] AdBanner(현재 placeholder)에 실제 광고를 붙일 경우, 맞춤형 광고를 위한 개인정보 처리가
      있다면 개인정보처리방침에 반영

**완료 조건**: 위 4개 카테고리 항목에 대해 변호사 검토를 거쳐 우선순위·구현 여부가 확정되고,
①(가입 연령)·②(약관·동의)·③(콘텐츠·안전) 카테고리의 필수 항목이 실제 배포 전에 구현된다.
④는 서비스 규모에 따라 유예 가능 여부를 함께 자문받는다.

---

## Phase 7.8 — 계정 탈퇴 버그 수정 + Phase 7.7 실사용 검증 (완료, 2026-08-15)

Phase 7.7(법적/규정 준수) 핵심 항목을 구현한 뒤 실사용 흐름으로 검증하는 과정에서 발견된
버그 2건, UX 개선 1건, 설계상 알려진 한계 1건을 정리한다.

- [x] 계정 탈퇴가 "계정 탈퇴에 실패했습니다"로 항상 실패하던 버그 수정
- [x] 계정 탈퇴 성공 후 화면에 예전 닉네임/로그아웃 버튼이 그대로 남아있던 버그 수정
- [x] Playwright로 이메일 가입 → setup-profile 강제 이동 → 약관 미동의/미성년자 차단 →
      정상 가입 → 닉네임·방 제목 금칙어 차단까지 실제 재현 검증
- [x] "기존 계정은 약관 동의 관문을 거치지 않는다"는 한계를 발견 — 서비스 미오픈 상태라
      별도 백필 로직 없이 배포 시점 계정 초기화로 해결하기로 결정(§Phase 9)
- [x] setup-profile 화면에 로그아웃 버튼이 없어 완료 전까지 빠져나갈 방법이 없던 문제 수정

**① 계정 탈퇴 실패 버그**: `admin_audit_logs.admin_id` 컬럼이 `not null`이면서 동시에
`references auth.users(id) on delete set null`로 정의되어 있어 설계가 모순이었다
(§migration 20260810020000). 관리자 조치(신고 처리 등)를 한 번이라도 수행해 자신의
`admin_id`로 남은 감사 로그가 있는 계정이 스스로 탈퇴하면, `auth.users` 삭제 시 FK가 해당
로그의 `admin_id`를 null로 바꾸려다 NOT NULL 제약에 걸려 삭제 트랜잭션 전체가 실패했다
(실사용 중 재현, Auth 로그에서 `ERROR: null value in column "admin_id" ... violates
not-null constraint` 확인). `admin_id` 컬럼을 nullable로 바꿔 `on delete set null`이
원래 의도대로 동작하게 했다 — 관리자 계정이 탈퇴해도 감사 로그(조치가 있었다는 사실)는
남고 누가 했는지만 익명화된다. 마이그레이션:
`20260815010000_fix_admin_audit_logs_admin_id_not_null_conflicts_with_on_delete_set_null.sql`.
격리된 테스트 계정으로 재현 후 정상 삭제되는 것을 SQL로 직접 확인했다.

**② 탈퇴 후 화면에 로그인 상태가 남는 버그**: `components/delete-account-button.tsx`가
탈퇴 성공 후 `router.push("/")`만 호출했는데, `(main)/layout.tsx`가 `/profile`과 `/`가
공유하는 레이아웃이라 Next.js가 이미 렌더링해둔(로그인 상태 포함) 레이아웃을 그대로
재사용해 헤더·사이드바가 갱신되지 않았다. `router.refresh()`를 추가해 현재 라우트의 서버
컴포넌트 트리를 강제로 다시 가져오도록 수정, Playwright로 탈퇴 직후 "로그인/회원가입"
버튼으로 즉시 전환되는 것을 확인했다.

**③ 기존 계정의 약관 동의 미적용 (알려진 한계, 의도적으로 보류)**: `app/(main)/layout.tsx`의
관문은 "닉네임이 없는 로그인 사용자"만 setup-profile로 보내므로, Phase 7.7 작업 이전에
이미 닉네임을 설정해둔 기존 계정(`kyu275` 등 개발 중 테스트 계정)은 이 화면을 다시 볼 일이
없어 약관 동의를 받은 적이 없다. 아직 서비스를 실제로 운영 중이 아니므로 기존 사용자를
위한 별도 백필/재동의 로직을 만들지 않기로 결정 — 대신 실제 배포 시점에 남아있는 모든
계정을 삭제해 초기화하면, 이후 생기는 모든 계정은 예외 없이 신규 가입 경로(=약관 동의
관문)를 거치게 되므로 별도 마이그레이션 없이 해소된다(§Phase 9 체크리스트에 항목 추가).

**④ setup-profile 탈출구 없음 (UX 개선)**: 이 화면은 `app/auth/`(=`(main)` 레이아웃 밖)에
있어 헤더·사이드바가 없고, 로그아웃 버튼도 없었다. "닉네임 없는 로그인 사용자는 무조건
이 화면으로" 가드 특성상, 프로필 설정을 완료하지 않고 이탈하면 재로그인 시에도 계속 이
화면으로만 돌아와 다른 계정으로 전환하거나 그냥 나갈 방법이 없는 막다른 골목이었다(실사용
중 발견). `app/auth/setup-profile/page.tsx`에 기존 `signOut()` 서버 액션을 재사용한
"다른 계정으로 로그인하기 (로그아웃)" 링크를 추가해 해소했다. Playwright로 로그인 →
setup-profile 진입 → 로그아웃 버튼 클릭 → 홈으로 정상 이동까지 재현 검증했다.

모두 `npm run check-all` 통과 확인, Playwright + 격리된 테스트 계정/SQL로 재현 검증했다.

---

## Phase 7.9 — 실사용 피드백 3건 반영 + 나가기 버그 수정 (완료, 2026-08-15)

실사용 중 발견한 UX 개선 요청 3건(방채팅 목록 온라인 필터링, 아바타 파일 업로드, 프로필
웹사이트 필드 제거)을 구현하는 과정에서 심각한 버그 2건(방목록 크로스 계정 비공개 버그,
나가기 무동작 버그)이 추가로 발견되어 함께 수정했다.

- [x] 방채팅 목록에 "접속 중인 멤버가 한 명도 없는 방"은 노출하지 않도록 필터링 추가
- [x] `use-heartbeat.ts`를 탭 가시성과 무관하게 항상 60초 간격으로 갱신하도록 변경
- [x] 프로필 아바타를 URL 직접 입력 대신 내 PC 파일 업로드 방식으로 변경
- [x] 프로필 수정 화면에서 "웹사이트" 필드 제거
- [x] (버그) 방목록 온라인 필터링이 비멤버 계정에서 방을 전부 안 보이게 만들던 RLS 버그 수정
- [x] (버그) 방 "나가기"가 즉시 재입장되어 무동작처럼 보이던 버그 수정

**① 방채팅 목록 온라인 필터링**: 방이 많아지면 아무도 접속하지 않은 "죽은 방"들 사이에서
실제 대화 중인 방을 찾기 어려워진다는 지적에 따라, `getRoomList()`가 반환하기 전에 멤버
중 사이트 전역 온라인(`profiles.last_seen_at`) 기준으로 최근 1분 이내 접속한 사람이
한 명도 없는 방을 걸러내도록 했다(`lib/queries/rooms.ts`의 `ROOM_ONLINE_THRESHOLD_MS`).
검색 기능이 쓰는 기존 2분 임계값과는 별도 상수로 분리했다 — 방목록은 더 즉각적인 반영이
필요하다고 판단. 임계값을 하트비트 갱신 주기(60초)와 맞추면서, 탭이 백그라운드에 있어도
온라인으로 유지되는 게 자연스럽다는 논의 끝에 `use-heartbeat.ts`의 `visibilitychange`
기반 시작/정지 로직을 제거하고 항상 60초 인터벌로 갱신하도록 단순화했다(모바일에서 OS가
백그라운드 탭의 JS 실행을 정지시키는 경우는 하트비트가 끊겨 임계값 경과 후 자연스럽게
오프라인으로 수렴하므로 별도 처리 불필요). "내가 참여 중인 방" 목록(`getMyRoomList()`)은
의도적으로 필터링하지 않았다 — 다른 멤버의 접속 여부와 무관하게 내 대화로는 항상 돌아올
수 있어야 한다.

**② 아바타 파일 업로드**: `profiles.avatar_url`은 지금까지 순수 외부 URL 문자열로만
쓰여 아바타 전용 Storage 버킷이 없었다. 새 공개(public) 버킷 `avatars`를 만들고
(`supabase/migrations/20260815020000_create_avatars_bucket_and_rls.sql`), 사용자당
경로를 `{user_id}.{ext}` 하나로 고정해 재업로드 시 upsert로 자연스럽게 덮어쓰도록 했다.
`lib/storage/avatars.ts`에 브라우저에서 직접 업로드하는 헬퍼를 추가하고,
`components/profile-edit-form.tsx`의 URL 텍스트 입력을 파일 선택 + 실시간 미리보기
UI로 교체했다.

**③ 웹사이트 필드 제거**: 프로필 수정 화면·스키마(`lib/schemas/profile.ts`)·서버 액션
(`app/actions/profile.ts`)에서 입력·저장 경로만 제거했다. DB 컬럼(`profiles.website`)은
다른 곳에서 참조하지 않는 것을 확인했고 되돌리기 쉬운 축소라 그대로 남겨두었다.

**④ (버그) 방목록 크로스 계정 비공개**: ①을 구현하며 `getRoomIdsWithOnlineMember()`가
`room_members`를 `profiles`와 조인해 직접 조회하도록 했는데, `room_members` SELECT RLS
정책("members can view their room roster")이 "그 방의 멤버만 그 방의 room_members 행을
볼 수 있다"로 되어 있어 아직 참여하지 않은 방을 둘러보는 사용자에게는 항상 빈 결과가
반환되어 **모든 방이 목록에서 사라지는** 버그가 있었다(실사용 중 "다른 계정으로는 방목록이
안 보인다"는 보고로 발견). `is_room_member()`와 동일한 패턴으로 RLS를 우회하는
`SECURITY DEFINER` 함수 `rooms_with_online_member()`를 새로 만들어
(`supabase/migrations/20260815030000_create_rooms_with_online_member_function.sql`)
해결했다. 두 계정으로 교차 검증 완료.

**⑤ (버그) 방 나가기 무동작**: "나가기"를 눌러도 에러 없이 방목록으로 이동은 하지만 실제로는
멤버 상태·인원수가 그대로였다. DB 함수 자체는 정상 동작했고(디버그 로깅으로 DELETE가
정확히 1행 지워지는 것까지 확인), 문제는 **Next.js가 Server Action 호출 직후 그 액션이
호출된 현재 페이지(방 상세 페이지)를 자동으로 다시 렌더링**하는 데 있었다 — 이 시점이
클라이언트의 `router.push("/rooms")`보다 먼저 실행되어, 방 상세 페이지의 "공개방 즉시입장"
로직(비멤버면 `join_room` 자동 호출)이 나가기 직후 곧바로 재실행되며 조용히 재입장시키고
있었다. `app/actions/rooms.ts`의 `leaveRoomAction`을 기존 `signOut()`과 같은 패턴으로
바꿔, 성공 시 클라이언트로 결과를 반환하는 대신 서버 액션 내부에서 바로 `redirect("/rooms")`
하도록 수정해 해결했다(`redirect()`는 현재 라우트를 다시 렌더링하지 않고 곧장 새 경로로
이동시킨다). 재현·검증 과정에서 방 카드 링크에 `prefetch={false}`도 함께 적용해, 카드가
뷰포트에 보이기만 해도 방 상세 페이지가 미리 렌더링되어 조용히 재입장되는 잠재적 문제를
예방했다.

모두 `npm run check-all` 통과 확인, Playwright + 격리된 테스트 계정으로 교차 계정
시나리오까지 재현 검증했다.

---

## Phase 8 — UI/UX 마감 및 반응형 (완료, 2026-08-16)

Phase 2에서 잡은 뼈대에 실제 데이터가 다 연결된 상태에서, 세밀한 반응형 엣지 케이스와 마감 디테일을 다듬는다.

- [x] 홈 화면 최종 브랜딩 점검
- [x] 모바일/PC 레이아웃 세부 반응형 점검 (겹침/잘림 확인)
- [x] 메시지 리스트 스크롤/롱텍스트/이미지 렌더링 엣지 케이스
- [x] 강퇴·종료·탈퇴 등 주의 액션 UI 최종 검수
- [x] 접근성/로딩·에러 상태 점검

**연관 PRD**: §6.1, §6.2, §7.2 반응형

**착수 방식**: 체크리스트 항목명만으로는 구체적 대상이 없어서, 착수 전 Explore 에이전트 3개를
병렬로 띄워 코드베이스를 실제로 조사했다(반응형 레이아웃 / 로딩·에러 상태 / 위험 액션
UI·브랜딩). "확인해봤지만 이미 정상"인 항목(홈 화면 카피, 브랜드명 표기 일관성, 이미지
렌더링 aspect-ratio 처리, 관리자 위험 액션 스타일 일관성 등)은 그대로 두고, 실제로 존재하는
문제만 골라 수정했다. 인증 폼(로그인/회원가입 등) 4개의 에러 표시 방식이 다른 폼들과 다른
것도 발견했지만 인증 흐름은 손대지 않기로 하고 이번 범위에서는 제외했다.

**① 브랜딩**: `app/layout.tsx`가 스타터킷 초기 메타데이터(`title: "Next.js and Supabase
Starter Kit"`, `<html lang="en">`)를 그대로 쓰고 있었고, 파비콘·OG 이미지도 Next.js 기본값
그대로였다. `app/icon.tsx`/`app/opengraph-image.tsx`를 `next/og`의 `ImageResponse`로 새로
만들어 브랜드 컬러(`--brand`의 실제 sRGB 값 `#5358ee`, Playwright로 직접 계산) 배경에
말풍선 아이콘 + "수다온" 워드마크를 렌더링한다. 한글 텍스트는 satori 기본 폰트에 글리프가
없어 안 그려지는 문제가 있어, Google Fonts CSS2 API를 구형 User-Agent로 요청해 woff
포맷(`next/og`가 지원하는 형식) 폰트를 텍스트 서브셋으로 받아오는 헬퍼
(`lib/utils/og-font.ts`)를 추가했다. 기존 기본 `favicon.ico`/`opengraph-image.png`/
`twitter-image.png`는 삭제, `<html lang="ko">`로 수정, `package.json` name도 정리.

**② 반응형 레이아웃**: 이전 세션에서 방채팅/랜덤채팅 "대화 중" 화면(`RoomChatView`/
`RandomChatView`)은 `(main)` 레이아웃의 `AppHeader`/`BottomNav`와 겹치는 문제로 `app/(chat)/`
그룹으로 옮겼는데, 매칭 **대기** 화면(`RandomMatchingClient`, `/random`)은 그때 누락되어
같은 이중 헤더/겹침 버그가 남아있었다. `app/(main)/random/page.tsx`를
`app/(chat)/random/page.tsx`로 옮기고 `min-h-screen`을 `h-dvh`로 맞췄다.

**③ 메시지 버블 롱텍스트 오버플로**: 공백 없는 긴 문자열(긴 URL 등)을 보내면 버블이
`max-w-[75%]`를 무시하고 화면 밖으로 넘치는 버그를 발견해 고쳤다. 원인은 한 곳이 아니라
flex 중첩 구조 전체에 걸쳐 있었다 — `min-w-0` 하나만 추가해서는 해결되지 않았는데, 내 메시지
정렬에 쓰는 `items-end`가 flex의 기본 `stretch` 동작을 깨서 안쪽 행(bubble+시각 wrapper)이
부모의 너비 제약과 무관하게 콘텐츠 크기(예: 950px+)로 그대로 커져버리는 게 진짜 원인이었다.
Playwright로 각 flex 레벨의 실제 computed width를 직접 찍어보고서야 발견했고,
`components/chat/chat-message-bubble.tsx`에 `min-w-0`(여러 레벨) + `max-w-full`(items-end로
stretch가 깨지는 레벨)을 함께 적용해 해결했다. 375px/320px 뷰포트에서 긴 URL 메시지와 이미지
버블 모두 정상 줄바꿈/크기 확인 완료.

**④ 위험 액션 UI**: 방 나가기·강퇴·계정 탈퇴·관리자 액션은 전부 확인 다이얼로그가 있는데
랜덤채팅 "종료"만 클릭 즉시 대화를 끝내고 있었다. `components/random/end-session-dialog.tsx`
신규(기존 `LeaveRoomDialog`와 동일 패턴)로 확인 절차를 추가했다.

**⑤ 로딩/에러 상태**: 앱 전체에 `error.tsx`가 하나도 없어 서버 예외 시 Next.js 기본
비브랜딩 에러 화면이 뜨는 문제를 발견해, 기존 `EmptyState` 컴포넌트를 재사용한 전역
`app/error.tsx` 1개를 추가했다(라우트별 세분화는 하지 않기로 결정). `app/(main)/rooms`,
`app/(main)/profile`에는 실제 서버 데이터 조회가 있는데 `loading.tsx`가 없어 느린 연결에서
빈 화면이 뜨는 문제도 있어 스켈레톤을 추가했다(채팅방/랜덤채팅 상세 화면은 "로딩 스켈레톤을
보여주지 않는다"는 이전 세션의 결정을 그대로 유지, 관리자 페이지는 저트래픽 1인 운영 도구라
제외). 조용히 묻히던 에러 2건도 고쳤다 — 이전 대화 페이지네이션 실패 시 아무 표시 없이
"더 이상 없음"으로 보이던 것(`lib/realtime/messages.ts`), 사용자 검색 실패 시 아무 반응이
없던 것(`components/search/user-search-panel.tsx`) 모두 `showError` 토스트를 추가했다.
아이콘 전용 버튼(`components/theme-switcher.tsx`)에 빠져있던 `aria-label`도 추가.

모두 `npm run check-all`/`npm run build` 통과 확인, Playwright로 각 수정 사항을 실제
뷰포트(375px/320px)와 프로덕션 빌드에서 재현·검증했다.
...
- [x] 최종 출시 수용 기준(§7.2) 전체 재검증
- [x] 배포 후 오래된 탭의 구버전 클라이언트 감지 및 새로고침 유도 (버전 스큐 처리) — `components/version-watcher.tsx`
- [ ] 커스텀 도메인(`www.sudaon.com`) 연결 — 도메인 미구매로 대기 중, Vercel 기본 도메인으로 우선 운영
...
## Phase 9.1 — 배포 후 실사용 버그 수정 및 랜덤채팅 이탈 감지 개선 (완료, 2026-08-16)

프로덕션 배포(Vercel) 이후 실사용 중 발견된 버그 3건을 수정하고, 그 과정에서 랜덤채팅의
"상대방 이탈 감지" 설계를 여러 차례 갈아엎었다.

**① (버그) 게스트 익명 세션이 로그인된 것처럼 잘못 표시됨**: 랜덤채팅을 한 번이라도 써본
브라우저가 이후 모든 페이지에서 "로그아웃"/"닉네임 없음"으로 잘못 표시되는 문제. 랜덤채팅은
`signInAnonymously()`로 익명 세션을 발급하는데(§ARCHITECTURE 2.2), 이 익명 사용자는
`profiles`가 아니라 `guest_profiles`에 저장된다(§20260804145603). `app/(main)/layout.tsx`와
`app/(main)/rooms/page.tsx`가 로그인 여부를 `Boolean(user)`로만 판단해 익명 세션도 로그인으로
오판했다 — `Boolean(profile)`/`is_anonymous` 기준으로 수정.

**② (버그) 랜덤채팅 매칭 직후 즉시 "상대방이 대화를 종료했습니다"로 끊김**: 실사용 중
재현·Supabase 로그 추적으로 원인을 특정했다. `lib/realtime/random.ts`가 서버가 내려준 상대의
마지막 활동 시각(timestamptz)을 클라이언트의 `Date.now()`와 직접 비교하던 방식이 매칭 직후
즉시 "상대가 오래 전에 멈췄다"고 오판하는 근본 원인이었다 — `end_random_session()` 호출
주체를 추적해보면 매번 자기 자신의 판단 오류였다. `heartbeat_random_session()` DB 함수가
"상대가 N초 넘게 조용한가"를 서버 시각 기준 boolean으로 직접 판단해 반환하도록 바꿔 클라이언트
시계/타임존 파싱과 완전히 분리했다.

이 과정에서 상대 이탈 감지 자체의 속도도 여러 단계로 개선했다:
- 처음엔 Presence(`leave` 이벤트)만으로 즉시 종료했으나, 매칭 직후 채널 재연결 타이밍에
  상대가 멀쩡한데도 가짜 leave가 뜨는 오탐이 있어 일단 제거하고 정기 하트비트(10초/25초,
  최대 35초)만으로 되돌렸다.
- Presence를 "즉시 확정"이 아니라 "힌트 → 짧게 대기 후 하트비트로 재검증" 구조로 다시
  붙였다 — leave를 받아도 곧장 끊지 않고 10초 기다렸다가 더 짧은 임계값(8초)으로 재확인,
  그 사이 상대 하트비트가 오면 무시(가짜 신호), 재연결(`join`)이 오면 재검증 자체를 취소.
- 탭 종료/페이지 이동 시 `pagehide` + `navigator.sendBeacon()`으로 "종료" 버튼을 누른 것과
  동일하게 즉시 종료하는 `/api/random/beacon-end` 경로도 추가했다(best-effort 보너스 경로).
- 그런데도 실측 결과 개선폭이 기대에 못 미쳤다 — Presence leave 자체가 Supabase Realtime의
  내부 25초 하트비트에 종속돼 있어 느렸고(웹서치로 확인, 유료/무료 플랜과 무관), sendBeacon도
  실사용 브라우저에서 서버에 아예 도착하지 않는 경우가 재현됐다(방채팅 자동종료 기능에서
  이미 겪은 것과 같은 종류의 불안정성, §Phase 7의 원복 사례와 동일 패턴). 결국 확실히 통제
  가능한 정기 하트비트 자체의 주기/임계값을 5초/12초로 좁혀 최악의 경우 감지 시간을 35초에서
  약 17초로 줄이는 쪽이 가장 신뢰할 수 있는 개선이었다(실측 약 18~20초). Presence·sendBeacon
  경로는 "되면 더 빠른" 보너스로 그대로 남겨뒀다.

**③ (개선) 방채팅 화면의 중복 하트비트 통합**: `(chat)/layout.tsx`가 방채팅 화면에서도
전역 하트비트(`HeartbeatProvider`, 60초, `profiles.last_seen_at`)를 무조건 마운트하고 있었는데,
방채팅은 이미 `useRoomHeartbeat`(60초, `profiles.room_heartbeat_*`)를 따로 돌리고 있어 같은
사용자의 같은 `profiles` 행에 완전히 독립적인 60초 타이머 2개가 겹쳐 돌며 Vercel 함수 호출/
Supabase 쓰기를 두 배로 만들고 있었다. `heartbeat_room_presence()` RPC가 `room_heartbeat_*`,
`last_seen_at`, DAU 집계(`record_daily_activity()`)를 한 호출로 묶도록 바꾸고, `(chat)/layout.tsx`
에서 전역 하트비트 마운트를 제거했다 — 랜덤채팅 화면은 `RandomChatView`가 `useHeartbeat()`를
직접 마운트해 기존 동작을 그대로 유지한다.

**트래픽 영향도 점검**: 이 개선들을 계기로 프로젝트 전체의 하트비트/폴링 6곳을 전수 조사하고
Supabase/Vercel 무료 플랜 영향도를 계산했다. Supabase DB 자체는(단일 row UPDATE 수준이라)
문제없지만, **Vercel 함수 호출(무료 100만 건/월)이 랜덤채팅 하트비트 때문에 예상보다 빨리
바닥날 수 있다** — 5분짜리 랜덤채팅 1건 = 하트비트만 120회 호출, 하루 300건(월 9,000건)
정도만 돼도 하트비트만으로 월 108만 건에 도달한다. 지금 트래픽으로는 문제없지만, 실사용자가
붙기 시작하면 Vercel 대시보드의 함수 호출 추이를 주기적으로 확인해야 한다. Hobby(무료)는
월 100만 건이 하드 캡이라 넘으면 서비스에 지장이 생기지만, Pro($20/월)는 하드 캡이 없고
초과분이 100만 건당 $0.60로 종량 과금돼 이 정도 규모라면 월 $1~10 수준의 추가 비용으로
해결된다 — 실사용자가 붙기 시작하는 시점에 Pro 전환을 우선 검토하는 게 하트비트 주기를 다시
늘리는 것보다 간단한 해법이다.
...
| 7.7 | 법적/규정 준수 (배포 전 필수) | 핵심 항목 구현 완료(법률 검토·이미지 검열 결정 대기) |
| 7.8 | 계정 탈퇴 버그 수정 + Phase 7.7 실사용 검증 | 완료 |
| 7.9 | 실사용 피드백 3건(방목록 필터링/아바타 업로드/웹사이트 제거) + 나가기 버그 수정 | 완료 |
| 8 | UI/UX 마감·반응형 | 완료 |
...
| 13 | 검색 노출(SEO) | 기술 작업 + GSC/네이버 등록 완료, 콘텐츠·외부 유입은 지속 운영 |
