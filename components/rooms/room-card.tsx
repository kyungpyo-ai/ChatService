 ...
 * 방이 많아져도 목록을 스캔하기 쉽도록 2줄(제목/인원 · 방장 정보/생성 시각) 구성으로 높이를 줄였다.
 */
export function RoomCard({ room, currentUserId }: RoomCardProps) {
  ...
  return (
    <TransitionLink
      ...
      className={
        isMine
          ? "bg-surface hover:bg-surface-muted border-brand block rounded-(--radius-card) border-2 p-3 shadow-(--shadow-card)"
          : "bg-surface hover:bg-surface-muted block rounded-(--radius-card) border p-3 shadow-(--shadow-card)"
      }
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex shrink-0 items-center gap-1.5">
          {room.isPrivate && (
            <span className="bg-brand-muted text-brand flex h-5 w-5 items-center justify-center rounded">
              <Lock size={12} />
            </span>
          )}
